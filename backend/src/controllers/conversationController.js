...
export const getUserConversationForSocketIO = async (userId) => {
  try {
    ...
    console.error('Lỗi khi fetch conversation for socket io:', error);
    return [];
  }
};
...
export const markAsSeen = async (req, res) => {
  try {
    ...
    if (!last) {
      return res
        .status(200)
        .json({ message: 'Không tồn tại tin nhắn để đánh dấu đã xem' });
    }

    if (last.senderId.toString() === userId) {
      return res
        .status(200)
        .json({ message: 'Tin nhắn không cần đánh dấu đã xem' });
    }
