  ...
  try {
    ...
    if (recipientId) {
      ...
      if (!isFriend) {
        return res
          .status(403)
          .json({ message: 'Bạn chỉ có thể gửi tin nhắn cho bạn bè' });
      }
      return next();
    }
    ...
    console.error('Lỗi xảy ra khi kiểm tra trạng thái bạn bè:', error);
...
export const checkConversationMembership = async (req, res, next) => {
  try {
    ...
    console.error('Lỗi xảy ra khi kiểm tra thành viên cuộc trò chuyện:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống' });
  }
};
